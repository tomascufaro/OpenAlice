 ...
 * narrow: confirmed bearer-only gateways (MiniMax, LongCat, and OpenRouter's
...
export function resolveAnthropicAuthMode(
  ...
  if (/api\.minimaxi\.com|api\.minimax\.io|api\.longcat\.chat|openrouter\.ai/i.test(opts.baseUrl ?? '')) return 'bearer'
  return 'x-api-key'
}
