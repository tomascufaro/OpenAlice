...
export interface GuardianControlServerOptions {
  homeRoot: string
...
export interface GuardianControlServer {
  endpoint: string
  close: () => Promise<void>
}
...
export function guardianControlEndpoint(
  homeRoot: string,
...
export async function startGuardianControlServer(
  options: GuardianControlServerOptions,
): Promise<GuardianControlServer> {
  ...
  const server = createServer((socket) => {
    ...
    const finish = (response: unknown, afterWrite?: () => void) => {
      if (handled) return
      handled = true
      socket.end(`${JSON.stringify(response)}\n`, afterWrite)
    }
    const fail = (id: string | null, code: string, message: string) => finish({
    ...
    socket.on('data', (chunk) => {
      ...
      try {
        request = JSON.parse(body.slice(0, newline)) as Record<string, unknown>
      } catch {
  ...
  if (platform !== 'win32') {
    ...
    socketIdentity = await endpointFileIdentity(endpoint)
  }
  ...
  return {
    ...
    close: async () => {
      if (closed) return
      closed = true
      const serverClosed = new Promise<void>((resolvePromise) => server.close(() => resolvePromise()))
      for (const socket of sockets) socket.destroy()
      await serverClosed
      if (platform !== 'win32' && socketIdentity !== null) {
        const currentIdentity = await endpointFileIdentity(endpoint).catch(() => null)
...
function probeEndpoint(endpoint: string, timeoutMs: number): Promise<void> {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    socket.setTimeout(timeoutMs, () => {
      const error = new Error(`Timed out connecting to ${endpoint}`) as ErrorWithCode
...
async function endpointFileIdentity(endpoint: string): Promise<string> {
...
async function waitForEndpointPath(endpoint: string): Promise<void> {
